...
function set_click_actions(staticUrl, metexploreInfoUrl){
    ...
    $('#enlarge').click(function() {
        if (fullScreen){
            ...
            $('.pals-table_wrapper_toolbar').css('top',155);
            ...
            $('.pals-table_wrapper_toolbar').css('top',41);
    ...
    $('#right-side-button').click(function (e) {
        if (right_panel) {
            ...
            $('.pals-table_wrapper_toolbar').width('100%');
            ...
            $('.pals-table_wrapper_toolbar').width('80%');
...
function set_peaktable(url, samplesGroupsNum, callback){
    ...
    var numCols = peakTable.columns().nodes().length;
    peakTable.column(numCols - 3).visible(false);
    peakTable.column(numCols - 2).visible(false);
    peakTable.column(numCols - 1).visible(false);
...
function set_palstable(url, numComps){

    // numComps is the number of comparisons in the analysis - we will need this X2 for the pals table
    // There are ALWAYS 5 columns that precede the p-vales for this table (see experiments.views.get_pals_table)
    // and we need to double the number of comparisons for the PAL and combined p-vales
    var numComparisons = numComps*2;
    var numComps = numComps
    var excludeColIdx = [0, 1, 2, 3, 4, 5]; //These are the first 6 columns that always precede the p-value data
    var EAIdxArray = [] //Array to hold the position of the EA p-values
    var PALSIdxArray = [] //Array to hold the position of the PALS p-values
    const PRE_COLUMNS = 6

    for (var n=0; n<numComps; n++) {
        EAIdxArray.push(6 + n);
    }

    var PALS_start = (EAIdxArray.slice(-1)[0])+1 //holds the starting position of the PALS p-values

    console.log("EAIdxArray ", EAIdxArray  )

    for (var n=0; n<numComps; n++){
        PALSIdxArray.push(PALS_start+n)
    }

    console.log("PALSIDxArray ", PALSIdxArray)

    // Compute the indices of the comparison p-value columns (PALS and EA) in the table
    var comparisonIdxArray = []
    for (var n=0; n<numComparisons; n++) {
        comparisonIdxArray.push(PRE_COLUMNS + n);
    }

    //Initially exclude all the columns from being selected by the colVis button
    excludeColIdx.push.apply(excludeColIdx, comparisonIdxArray)

    console.log("comparisonIdxArray ", comparisonIdxArray)
    console.log("excludeColIdx ", excludeColIdx)

    // To colour the table we need one
    var colourIdxArray  = comparisonIdxArray.map(function(value) {
    return value -2;
    } );

    // Prepare variables for creating coloured p-values
    // low_p = greens   [pale-green to dark-green]
    var low_p_colours = ["#6bc033", "#50b822", "#33b112","#17aa03"];
    //mid_p = yellow [paler-green/yellow to pale-green]
    var mid_p_colours = ["#c9de86", "#b5d66f","#9ecf5a", "#85c746"];
    // high_p = reds [off white to pale yellow]
    var high_p_colours = ["#fcfaed", "#f4f3d1", "#eaedb7","#dbe59e" ];

    //Signigicant (or otherwise) p-values
    //If p-value is less that SIGP or V_SIGP
    const SIGP = 0.05;
    //const UN_SIGP = 0.5;
    const V_SIGP = 0.0005;
    //unsignificant is > 0.05....
    var high_fold_change_bins = [0.4, 0.2, 0.1, 0.05];
    //Bin used if less than 0.05
    var mid_fold_change_bins =  [0.04, 0.01, 0.005, 0.0005]
    //Bin used is less that 0.005
    var low_fold_change_bins = [0.00005, 0.0000005, 0.000000005, 0]


    var palsTable = $('#pals-table').DataTable( {
                    "sAjaxSource": url,
                    "sDom": '<"pals-table_wrapper_toolbar"CliT>rtp',
                    "oColVis": {
                        "sButtonText": "Switch display",
                        "aiExclude": excludeColIdx,
                        "aoGroups": [
                            {
                                "sTitle": "EA_results",
                                "aiColumns": EAIdxArray
                            },
                            {
                                "sTitle": "PALS_results",
                                //"aiColumns": groupIdxArray.concat(comparisonIdxArray)
                                "aiColumns": PALSIdxArray
                            }
                        ]
                    },
                    "createdRow": function (row, data, index) {
                    // Set the background-color of the p-value columns to match their values

                    // Choose the colour based on the logfc and apply it to the cell

                        for (var idx in comparisonIdxArray) {

                            var p_value = Math.abs(parseFloat(data[comparisonIdxArray[idx]]));
                            var colours = [];
                            var fold_change_bins = [];

                            if (p_value < V_SIGP) {
                                colours = low_p_colours;
                                fold_change_bins = low_fold_change_bins;
                                //console.log("p-value was > 0.0005! " + parseFloat(data[comparisonIdxArray[idx]]));
                            }
                            else if (p_value < SIGP) {
                                colours = mid_p_colours;
                                fold_change_bins = mid_fold_change_bins;

                                }
                            // The p-value is > SIGP (therefore unsignificant)
                            else {
                                colours = high_p_colours;
                                fold_change_bins = high_fold_change_bins;

                            }

                            var colour = null;
                            for (var i in fold_change_bins) {
                                if (p_value >= fold_change_bins[i])
                                {
                                    colour = colours[i];
                                    $('td', row).eq(colourIdxArray[idx]).css('background-color', colour);
                                    break;
                                }
                        }
                    }
                                ...
                                "aButtons":    [
                                    {
                                        ...
                                        "sButtonText": "csv",
                                        "mColumns": "visible",
                                        "sFileName": "pathway_activity.csv",
                                        "sToolTip": "Save as CSV"
                                    },
                                    {
                                        "sExtends": "xls",
                                        "sButtonText": "xls",
                                        "mColumns": "visible",
                                        "sFileName": "pathway_activity.xls",
                                        "sToolTip": "Save as XLS"
                                    }
                                ]
                            }
                    ...
                    "aoColumnDefs": [
                        ...
                        { // Columns with EA p-values
                        "aTargets": EAIdxArray,
                        },
                        { // Columns with PALS p-value
                        "aTargets": PALSIdxArray,
                        }],
    });
...
//A function to toggle the p-vales in the pals table but it didn't quite work.
// function fnShowHide(iCol)
// 			{
// 				/* Get the DataTables object again - this is not a recreation, just a get of the object */
// 				var oTable = $('#palsTable').dataTable();
//
